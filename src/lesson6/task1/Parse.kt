...
fun dateDigitToStr(digital: String): String {
    ...
    return try {
        ...
        if (parts.size != 3 || parts[1].toInt() !in 1..12) throw Exception()
...
fun flattenPhoneNumber(phone: String): String {
    ...
    return try {
        ...
        if (changedNumber != number.length) throw Exception()
        number
    } catch (e: Exception) {
        ""
    }
    ...
    for (element in parts) {
        if (element != "%" && element != "-") {
            for (it in element)
                if (!it.isDigit())
                    return -1
            val right = element.toInt()
            bestJump.add(right)
        }
    }
    return if (!bestJump.isEmpty()) bestJump.sorted().last()
    else -1
}
